...
static func create_Sharpnoiselite_datasource(seed:int) -> ProceduralWorldDatasource:
	...
	datasource.noise_config[datasource.noise_idx_elevation]=SharpNoiseLiteDatasource.NoiseObject.new(seed,0,6,0.0011,0.5,4.0)
	datasource.noise_config[datasource.noise_idx_main_elevation]=SharpNoiseLiteDatasource.NoiseObject.new(seed,0,9,0.0001,1,1)
	datasource.noise_config[datasource.noise_idx_moisture]=SharpNoiseLiteDatasource.NoiseObject.new(seed,1,4,0.05,3,0.4)
	datasource.noise_config[datasource.noise_idx_heat]=SharpNoiseLiteDatasource.NoiseObject.new(seed,2,4,0.05,3,0.4)
	
	datasource.noise_generators=[]
	datasource.noise_generators.resize(datasource.noise_config.size())
	for i in datasource.noise_config.size():
		datasource.noise_generators[i]=SharpNoiseLiteDatasource.create_noise_generator(datasource.noise_config[i])
