...
### CONSTRUCTORS #################
func _init():
	datasource=Worldmap.new()

func init_noises():
	for i in range(5):
		datasource.set_noise(i,noise_generators[i])
...
func get_biome_image(size:Vector2i):
	...
	var data = datasource.get_biome_image(size)
	current_area_info=fill_area_info_cache()
	area_info_cache.append(current_area_info)
	return data

func fill_area_info_cache():
	var biome_idx=datasource.get_current_biome()
	var heat=datasource.get_current_heat()
	var moisture=datasource.get_current_moisture()
	var elevation=datasource.get_current_elevation() 
